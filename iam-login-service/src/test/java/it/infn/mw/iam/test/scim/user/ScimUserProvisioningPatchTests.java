...
public class ScimUserProvisioningPatchTests {
  ...
  private ScimUserPatchRequest getPatchRequest(ScimPatchOperationType op, ScimUser updates) {

    ScimPatchOperation<ScimUser> userOp =
        (new ScimPatchOperation.Builder<ScimUser>()).op(op).value(updates).build();

    return ScimUserPatchRequest.builder().addOperation(userOp).build();
  }
  ...
  private void doPatch(ScimUserPatchRequest req, String location, HttpStatus expectedStatus) {
    ...
    given().port(8080).auth().preemptive().oauth2(accessToken).contentType(SCIM_CONTENT_TYPE)
        .body(req).log().all(true).when().patch(location).then().log().all(true)
  ...
  public void testPatchUserInfo() {
    ...
    ScimAddress address = ScimAddress.builder().country("IT")
        .formatted("viale Berti Pichat 6/2\nBologna IT").locality("Bologna").postalCode("40121")
        .region("Emilia Romagna").streetAddress("viale Berti Pichat").build();

    ScimName name = ScimName.builder().givenName("John jr.").familyName("Lennon II")
        .middleName("Francis").build();

    /*
     * Update: - email - username - active - name - address
     */
    ScimUser lennon_update =
        ScimUser.builder("john_lennon_jr").buildEmail("john_lennon_jr@email.com")
            .active(!lennon.getActive()).name(name).addAddress(address).build();

    ScimUserPatchRequest req = getPatchRequest(ScimPatchOperationType.add, lennon_update);

    doPatch(req, lennon.getMeta().getLocation(), HttpStatus.NO_CONTENT);

    given().port(8080).auth().preemptive().oauth2(accessToken).contentType(SCIM_CONTENT_TYPE).when()
        .get(lennon.getMeta().getLocation()).then().log().all(true)
        .statusCode(HttpStatus.OK.value()).body("userName", equalTo(lennon_update.getUserName()))
        ...
        .body("active", equalTo(lennon_update.getActive())).body("emails", hasSize(equalTo(1)))
  ...
  public void testPatchAddRessignAndRemoveOidcId() {
    ...
    ScimUserPatchRequest req = getPatchRequest(ScimPatchOperationType.add, updateOidcId);

    doPatch(req, lennon.getMeta().getLocation(), HttpStatus.NO_CONTENT);

    given().port(8080).auth().preemptive().oauth2(accessToken).contentType(SCIM_CONTENT_TYPE).when()
        .get(lennon.getMeta().getLocation()).then().log().all(true)
        .statusCode(HttpStatus.OK.value()).body("id", equalTo(lennon.getId()))
  ...
  public void testPatchRemoveNotExistingOidcId() {
    ...
    ScimUserPatchRequest req = getPatchRequest(ScimPatchOperationType.remove, lennon_update);
