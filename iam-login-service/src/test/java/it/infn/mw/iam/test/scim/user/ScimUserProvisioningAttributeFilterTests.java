...
public class ScimUserProvisioningAttributeFilterTests {
  ...
  public void testReuturnOnlyUsernameRequest() {
    ...
    given().port(8080).auth().preemptive().oauth2(accessToken).accept(SCIM_CONTENT_TYPE).log()
        .all(true).param("count", 1).param("attributes", "userName").when().get("/scim/Users")
        .then().log().all(true).statusCode(HttpStatus.OK.value()).body("totalResults", equalTo(250))
        ...
        .body("Resources[0].locale", is(nullValue())).body("Resources[0].timezone", is(nullValue()))
        .body("Resources[0].active", is(nullValue())).body("Resources[0].title", is(nullValue()))
  ...
  public void testMultipleAttrsRequest() {
    ...
    given().port(8080).auth().preemptive().oauth2(accessToken).accept(SCIM_CONTENT_TYPE).log()
        .all(true).param("count", 2)
        .param("attributes", "userName,emails," + ScimConstants.INDIGO_USER_SCHEMA).when()
        .get("/scim/Users").then().log().all(true).statusCode(HttpStatus.OK.value())
        .body("totalResults", equalTo(250)).body("itemsPerPage", equalTo(2))
        .body("startIndex", equalTo(1)).body("schemas", contains(ScimListResponse.SCHEMA))
