...
public class IndigoOIDCAuthFilter
   ...
   */
  protected void handleAuthorizationRequest(final HttpServletRequest request,
    ...
    if (issResp.shouldRedirect()) {
      ...
      if (!Strings.isNullOrEmpty(issResp.getTargetLinkUri())) {
        // there's a target URL in the response, we should save this so we can
        // forward to it later
        session.setAttribute(TARGET_SESSION_VARIABLE,
          issResp.getTargetLinkUri());
      }
      ...
      RegisteredClient clientConfig = clients
      ...
      if (clientConfig.getRegisteredRedirectUri() != null
        ...
        redirectUri = Iterables
          .getOnlyElement(clientConfig.getRegisteredRedirectUri());
      } else {
        // otherwise our redirect URI is this current URL, with no query
        // parameters
      ...
      Map<String, String> options = authOptions.getOptions(serverConfig,
        clientConfig, request);

      String authRequest = authRequestBuilder.buildAuthRequestUrl(serverConfig,
        clientConfig, redirectUri, nonce, state, options,
        issResp.getLoginHint());
  ...
  protected Authentication handleAuthorizationCodeResponse(
    ...
    ServerConfiguration serverConfig = servers.getServerConfiguration(issuer);
    final RegisteredClient clientConfig = clients
      .getClientConfiguration(serverConfig);
    ...
    form
      .setAll(authOptions.getTokenOptions(serverConfig, clientConfig, request));

    String redirectUri = getStoredSessionString(session,
      REDIRECT_URI_SESSION_VARIABLE);
    if (redirectUri != null) {
    ...
    if (SECRET_BASIC.equals(clientConfig.getTokenEndpointAuthMethod())) {
      ...
      restTemplate = new RestTemplate(factory) {
        ...
        @Override
        protected ClientHttpRequest createRequest(final URI url,
      ...
      if (SECRET_JWT.equals(clientConfig.getTokenEndpointAuthMethod())
        ...
        if (SECRET_JWT.equals(clientConfig.getTokenEndpointAuthMethod())
          ...
          signer = symmetricCacheService
            .getSymmetricValidtor(clientConfig.getClient());

        } else if (PRIVATE_KEY
          .equals(clientConfig.getTokenEndpointAuthMethod())) {
        ...
        claimsSet
          .audience(Lists.newArrayList(serverConfig.getTokenEndpointUri()));
        claimsSet.jwtID(UUID.randomUUID().toString());

        // TODO: make this configurable
        Date exp = new Date(System.currentTimeMillis() + (60 * 1000)); // auth
                                                                       // good
                                                                       // for 60
                                                                       // seconds
        ...
        JWSHeader header = new JWSHeader(alg, null, null, null, null, null,
          null, null, null, null, signer.getDefaultSignerKeyId(), null, null);
    ...
    try {
      jsonString = restTemplate
        .postForObject(serverConfig.getTokenEndpointUri(), form, String.class);
      ...
      try {
        ...
        if (clientAlg != null) {
          if (!clientAlg.equals(tokenAlg)) {
            throw new AuthenticationServiceException("Token algorithm "
              + tokenAlg + " does not match expected algorithm " + clientAlg);
          }
        }
        ...
        if (idToken instanceof PlainJWT) {
          ...
          if (tokenAlg.equals(JWSAlgorithm.HS256)
            ...
            jwtValidator = validationServices
              .getValidator(serverConfig.getJwksUri());
          }
        ...
        if (idClaims.getAudience() == null) {
          ...
          .contains(clientConfig.getClientId())) {
          throw new AuthenticationServiceException(
        ...
        // construct an PendingOIDCAuthenticationToken and return a
        // Authentication object w/the userId and the idToken

        PendingOIDCAuthenticationToken token = new PendingOIDCAuthenticationToken(
          idClaims.getSubject(), idClaims.getIssuer(), serverConfig, idToken,
   ...
   * 
   * @param request
   *          The request from which to extract parameters and handle the error
   * @param response
   *          The response, needed to do a redirect to display the error
   * @throws IOException
   *           If an input or output exception occurs
   */
  protected void handleError(final HttpServletRequest request,
    final HttpServletResponse response) throws IOException {
    ...
    throw new AuthenticationServiceException(
      "Error from Authorization Endpoint: " + error + " " + errorDescription
        + " " + errorURI);
  }
   ...
   * @return
   */
  private static String getStoredSessionString(final HttpSession session,
    final String key) {
   ...
   * @return
   */
  protected static String createNonce(final HttpSession session) {
   ...
   * @return
   */
  protected static String getStoredNonce(final HttpSession session) {
   ...
   * @return
   */
  protected static String createState(final HttpSession session) {
   ...
   * @return
   */
  protected static String getStoredState(final HttpSession session) {
  ...
  /**
   * Handle a successful authentication event. If the issuer service sets a
   * target URL, we'll go to that. Otherwise we'll let the superclass handle it
   * for us with the configured behavior.
   */
  ...
  public void setTimeSkewAllowance(final int timeSkewAllowance) {
  ...
  public void setValidationServices(
    final JWKSetCacheService validationServices) {
  ...
  public void setServerConfigurationService(
    final ServerConfigurationService servers) {
  ...
  public void setClientConfigurationService(
    final ClientConfigurationService clients) {
   ...
   *          the authRequestBuilder to set
   */
  public void setAuthRequestUrlBuilder(
    final AuthRequestUrlBuilder authRequestBuilder) {
  ...
  /**
   * @param authOptions
   *          the authOptions to set
   */
  public void setAuthRequestOptionsService(
    final AuthRequestOptionsService authOptions) {
  ...
  public void setSymmetricCacheService(
    final SymmetricKeyJWTValidatorCacheService symmetricCacheService) {
  ...
  public void setTargetLinkURIAuthenticationSuccessHandler(
    final TargetLinkURIAuthenticationSuccessHandler targetSuccessHandler) {
  ...
  public void setTargetLinkURIChecker(
    final TargetLinkURIChecker deepLinkFilter) {
